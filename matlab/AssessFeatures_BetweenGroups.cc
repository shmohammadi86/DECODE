...
mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	...
	vector_size = max(m, n);
	uvec cols_null(vector_size);
	if(mxIsSparse(prhs[1])) {
		mat temp = mat(armaGetSparseMatrix(prhs[1]));  
		for (i = 0; i < vector_size; i++) {
			cols_null(i) = (uword)(temp[i]-1);
		}		
	}
	else {
		double* ptr = (double *) reinterpret_cast<double*>(mxGetPr(prhs[1])); 
		for (i = 0; i < vector_size; i++) {
			cols_null(i) = (uword)(ptr[i]-1);
		}		
	}	
	...
	vec logPvals = AssessFeatures_betweenGroups(A, cols, cols_null, rand_sample_size, rand_sample_no, thread_no);
